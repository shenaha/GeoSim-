globals [
  ...
  percent-friendly ;; on the average, what percent of a turtle's neighbors
                   ;; are friendly?
...
turtles-own [
  ...
  friendly         ;; for each turtle, indicates whether a turtle is friendly
  friendly-nearby  ;; how many neighboring patches have a turtle which is friendly?
  unfriendly-nearby ;; how many neighboring patches have a turtle which is unfriendly?
...
to update-turtles
  ask turtles [
    ...
    set friendly-nearby count (turtles-on neighbors) with [ friendly >= 1 ]
    set unfriendly-nearby count (turtles-on neighbors) with [ friendly = 0 ]
    set total-nearby similar-nearby + other-nearby
    set happy? similar-nearby >= (%-similar-wanted * total-nearby / 100) AND friendly-nearby >= ((%-friendly-wanted) * total-nearby / 100)
...
to update-globals
  ...
  let friendly-neighbors sum [ friendly-nearby ] of turtles
  set percent-friendly (friendly-neighbors / total-neighbors) * 100
...
"" ""
PENS
"default" 1.0 0 -2674135 true "" "plot percent-friendly"
